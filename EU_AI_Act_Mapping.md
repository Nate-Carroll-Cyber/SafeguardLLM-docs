...
* **Human-in-the-Loop (HITL) Mode:** Safeguard LLM automatically intercepts borderline traffic, including prompts above the suspicious entropy floor of `3.8` but not above the configured adversarial cutoff, and routes it to a `PENDING_REVIEW` queue, fulfilling the mandate that humans must be able to review and intervene before an automated action executes.
* **Global System Pause (DEFCON 1):** The Act specifically requires a "stop button" or equivalent mechanism to safely halt operations. Safeguard LLM’s DEFCON 1 toggle halts automated forwarding, routes new Analyst Chat prompts to manual review during an attack, and stops active Bulk Ingest replay.
...
* **Defense-in-Depth Sanitization (Regex, Entropy, Complexity):** By blocking prompt injections, token dilution attacks, and obfuscated payloads before they hit the LLM, Safeguard LLM enforces the required cybersecurity resilience against adversarial manipulation.
...
* **PII & Secret Redaction:** By stripping SSNs, emails, phone numbers, and keys before provider inference or downstream responder calls, Safeguard LLM enforces **Data Minimization** (a core tenet of both the AI Act and GDPR), reducing the risk that unauthorized personal data enters an external LLM context window.
