...
| `IAM-14` Strong Authentication | Cognito user pool; REQUEST authorizer verifying signature against pinned JWKS, `iss`, `exp`, `nbf`, `token_use`, `client_id`, scope; RFC 8707 resource binding on the human path | `05-cognito`, `07-compute`, `08-api`, `src/authorizer/index.mjs` | **Implemented** — human path; **Partial** — machine path, see note |
...
**Note on `IAM-14` and audience binding.** RFC 8707 resource indicators bind an
access token to a named resource, so a token minted for one API cannot be used at
another. Cognito supports this on authorization-code grants only. It **cannot** be
requested on `client_credentials`, so machine tokens carry no `aud` at all and
client binding there is `client_id` alone.

That is a platform constraint rather than a configuration gap, and the correct
response is what the authorizer does: enforce audience on the human path, assert
its *absence* on the machine path, and treat the two as separate validation
profiles. An earlier revision of the architecture specification described resource
binding as available generally; it is not, and §4.3 of that document has been
corrected.

The residual is real. A machine credential leaked from a consuming organisation's
environment is replayable against this API and against any other API in the same
user pool that accepts the same client. Closing it requires sender-constrained
tokens (mTLS, since Cognito lacks DPoP) or a different authorization server.
