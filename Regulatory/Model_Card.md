...
- **Analyst Chat / safeguard configuration:** OpenAI-compatible base URL and model ID are backend-managed through `SAFEGUARDS_*` environment variables or a future secret manager. Protected backend execution rejects browser-supplied safeguard endpoint and model overrides. The current System Configuration `safeguardEffectivePrompt` is an allowlisted request field and is forwarded verbatim as the safeguard judge system prompt. The optional browser-memory `safeguardApiKey` field is retained for local LM Studio testing; when absent, the backend falls back to `SAFEGUARDS_API_KEY`.
