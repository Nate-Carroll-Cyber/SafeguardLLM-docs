# Technical Reference & Architecture Specification: Safeguard LLM
...
Safeguard LLM employs a **Shield-and-Sword** architectural pattern to secure Large Language Model (LLM) interactions. This multi-stage defense-in-depth strategy ensures that adversarial payloads are neutralized at the edge before reaching high-compute inference engines.
...
| :--- | :--- | :--- | :--- |
| **Backend Route** | Unexpected transport or server failure | **Fail-Secure** | The request is not forwarded; non-governed failures use plain HTTP error bodies rather than inventing a structured gateway status. |
...
*   **Thresholds:** Scores at or above the configured Syntactic Threshold (`65` by default, tunable from `40` through `90`) contribute to `Suspicious` classification; scores at or above `90` independently trigger `Adversarial` classification.
...
*   **Reasoning:** Safeguard LLM now treats concealment itself as hostile behavior in the governed prompt path, rather than as a lower-severity curiosity that waits for a second content-based match.
...
### 5.1 Authentication
Future external services would authenticate with the Safeguard LLM gateway using **Bearer Tokens (JWT)**.
...
| `userId` | `string?` | Optional requesting-user identifier. |
| `sessionId` | `string?` | Optional interaction-session identifier. |
| `metadata` | `object?` | Optional strict allowlist: `localReviewMode`, `source`, `providerLlmRoutingEnabled`, `responderLlmRoutingEnabled`, `instructionSimilarityEnabled`, `safeguardEffectivePrompt`, optional browser-memory `safeguardApiKey`, `instructionEmbedding`, and `instructionChunks`. Browser callers cannot choose backend provider endpoints, model IDs, or responder system prompts. |
| `tuning` | `object` | Optional per-request guardrail tuning accepted from authenticated callers: `entropyThreshold`, `syntacticThreshold`, `blockedKeywords`, `forbiddenTopics`, and `regexRules`. Frontend submissions populate this from the active governance configuration. |
...
| `401` | Plain error body | Missing or invalid bearer token returns `{"error":"Unauthorized protected route request."}`; `UNAUTHORIZED` is not a structured gateway status. |
...
| `502` | Plain error body | Downstream responder failure returns `{"error":"...","upstreamStatus":...}`; `RESPONDER_ERROR` is not a structured gateway status. |

The structured `/v1/intercept` status enum is exactly `CLEAN | QUEUED | INTERCEPTED | SHIELD_ERROR`.

Downstream responder outputs are output-sanitized before display. Safeguard telemetry and responder telemetry such as provider, model ID, status, latency, prompt hash, retry marker, token usage, prompt profile, context utilization, and output-sanitization flags are normalized back into Safeguard LLM audit records or structured gateway logs. Local responder passthrough is represented with responder status `DISABLED_LOCAL_ONLY` and model `local-responder-passthrough`; in that mode, OpenAI-compatible safeguard usage is used as the audit token source when present.
